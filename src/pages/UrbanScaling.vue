<template>
  <main class="max-w-3xl mx-auto p-6 space-y-6">
    

    <h1 class="text-3xl font-bold mt-4">Urban Scaling Laws</h1>
    <p>
      Most cities grow, but they don't grow evenly. Larger cities often have disproportionate increases in crime,
      income, and innovation. I study the dynamics of scaling laws — mathematical relationships that describe
      how various urban indicators grow with population size. These relationships have enormous potential in not 
      only filling the gaps in our understanding of the present but also preparing us for future resource needs. 
    </p>
    <p>
      Arrest data collected from law agencies acorss the U.S. is heavily missing due to a voluntary reporting program. 
      A major challenge remains identfying missing and underreported values.  
    </p>
      
    <img src="/images/projects/urban-scaling.jpg" alt="Missing Arrest Data" class="w-full rounded shadow" />
    <figcaption class="text-sm text-gray-500 mt-1">
      Estimated missingness (1980-2022, 42 years) in arrest data across U.S. counties
    </figcaption>
    <p>
      I leverage scaling laws to inform corrected arrest rates as well as uncertainty for 300 U.S. cities. 
      State-of-the-art Bayesian estimations allow modeling for time and region variations heirarchically, while maintainting transparency. 
    </p>

    <h1 class="text-1.5xl font-bold mt-2 mb-1">Publications</h1>
    <p class="italic">
      Changes in poverty and ethnic diversity predict the murder rate of US metropolitan statistical areas over a 60-year period <br>
      <span class="not-italic">(Second author, 2025, <em>Nature Human Behavior</em> — under review)</span>
    </p>

    <h1 class="text-1.5xl font-bold mt-2 mb-1">Code</h1>
    <p>
      The open-source data and code repository will be made available after publication.
    </p>

    
  </main>
</template>
